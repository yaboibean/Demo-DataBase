...
if customer_need.strip():
    ...
    if not results:
        ...
        for res in results:
            ...
            # Use st.markdown for the card container, but use st.write for the fields to avoid HTML/CSS conflicts
            ...
            st.write(f"**Date Uploaded:** {demo.get('Date Uploaded', 'N/A')}")
            st.write(f"**⭐ Similarity Score:** {res.get('similarity_score', 'N/A'):.3f}")
            st.write(f"**Reason:** {res.get('explanation', 'N/A')}")
            st.write(f"**Client Problem:** {demo.get('Client Problem', '')}")
            st.write(f"**Instalily AI Capabilities:** {demo.get('Instalily AI Capabilities', '')}")
            st.write(f"**Benefit to Client:** {demo.get('Benefit to Client', '')}")
